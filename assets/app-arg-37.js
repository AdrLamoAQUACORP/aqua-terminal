...
after A17 session termination.`
  ...
  function typeArg7Text(node, text, speed = 34) {
    return new Promise(resolve => {
      ...
      const step = () => {
        ...
        scheduleArg7Cinematic(step, speed + Math.random() * 18);
      };
  ...
  function openArg7FinalComposerCinematic() {
    ...
    if (!composer || !recipient || !text || !status || !send) return false;
  ...
  async function runArg7MailAutomation() {
    ...
    const finalText =
`Карта внутренних секторов Аквы — внутри пакета.
Окно пересменки охраны отмечено отдельно.

Пусть восстание пройдёт по плану. - Флин Эллада Данте`;

    await moveArg7FakeCursorTo(body, 650);
    await typeArg7Text(body, finalText, 30);
