...
after A17 session termination.`
  ...
  function getArg6AimLevel() {
    if (state.arg6AimScore >= 18 || state.arg6AimSeconds <= 10) return 4;
    if (state.arg6AimScore >= 12 || state.arg6AimSeconds <= 20) return 3;
    if (state.arg6AimScore >= 6 || state.arg6AimSeconds <= 32) return 2;
    return 1;
  }
  ...
  function getArg6AimConfig() {
    ...
    const configs = {
      1: { spawn: 950, life: 1900, fakeChance: 0.12, burstChance: 0.00, maxBurst: 1 },
      2: { spawn: 760, life: 1550, fakeChance: 0.24, burstChance: 0.16, maxBurst: 2 },
      3: { spawn: 590, life: 1250, fakeChance: 0.36, burstChance: 0.32, maxBurst: 3 },
      4: { spawn: 440, life: 980,  fakeChance: 0.48, burstChance: 0.46, maxBurst: 4 }
    };
  ...
  function updateArg6AimUI() {
    ...
    if (stage) {
      const labels = {
        1: "SLOW / SINGLE TARGETS",
        2: "FASTER / FIRST DECOYS",
        3: "MULTIPLE HOLES / MORE DECOYS",
        4: "MAXIMUM TRACE / RAPID SPAWN"
      };
  ...
  function missArg6RealHole(id) {
    ...
    state.arg6AimLink = Math.max(0, state.arg6AimLink - 11);
    arg6AimFlash("HOLE COLLAPSED // LINK -11", true);
    errorTone();
    updateArg6AimUI();
    ...
    if (target.real) {
      ...
      if (state.arg6AimScore === 6) {
        setArg6AimHint(
          ...
          "Теперь рядом начнут появляться ловушки WATCHDOG."
        );
      } else if (state.arg6AimScore === 12) {
        setArg6AimHint(
          "НЕ СПЕШИ КЛИКАТЬ ПО ВСЕМУ. ИЩИ ДВОЙНОЕ КОЛЬЦО.",
          "Фейковые дыры живут дольше и пытаются отвлечь тебя."
        );
      ...
      state.arg6AimLink = Math.max(0, state.arg6AimLink - 16);
  ...
  function scheduleArg6AimSpawn() {
    ...
    if (Math.random() < config.burstChance) {
      burst = 2 + Math.floor(Math.random() * Math.max(1, config.maxBurst - 1));
    }

    // Always guarantee at least one real target in burst.
    spawnArg6AimTarget(true);

    for (let i = 1; i < burst; i++) {
      setTimeout(() => spawnArg6AimTarget(null), i * 90);
    }

    const jitter = config.spawn * (0.82 + Math.random() * 0.30);
  ...
  function ensureArg7SummaryRow() {
    ...
    row.addEventListener("click", () => {
      ...
      if (!state.arg7FinalDraftRevealed) {
        state.arg7FinalDraftRevealed = true;
        localStorage.setItem("akwaos.arg7FinalDraftRevealed", "1");

        setTimeout(() => {
          ensureArg7FinalDraftRow();
          showMailNotification("ВОССТАНОВЛЕН ИСХОДЯЩИЙ ЧЕРНОВИК", "OUTBOX // [UNSENT]");
        }, 900);
      }
    });
    ...
    content.innerHTML = `
      <div class="arg7-final-mail">
        <div class="arg7-final-mail__meta">
          ...
          <span>TO:</span><b id="arg7-final-recipient" class="arg7-final-recipient">N▉C▓-??//▒▒▒</b>
          <span>SUBJECT:</span><b>AKWA / NIGHT ACCESS PACKAGE</b>
        </div>

        <div class="arg7-final-mail__body">
          <p>Карта внутренних секторов Аквы восстановлена.</p>
          <p>Ночной маршрут через A-17 и SEC-C подтверждён.</p>
          <p>Записи о переводах и доступах — внутри пакета.</p>
          <p>Данные по Астери Данте найдены в материалах эксперимента №626.</p>
