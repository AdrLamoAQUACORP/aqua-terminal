(() => {
  ...
  const state = {
    ...
    arg6TraceTimer: null,
    arg6MemoryComplete: localStorage.getItem("akwaos.arg6MemoryComplete") === "1",
    arg6MemoryRotations: [0, 0, 0, 0, 0, 0, 0, 0, 0],
...
REF: BIO-CACHE / AQ-S17-441`,
  ...
  function handleCommand(raw) {
    ...
    if (cmd === "arg6memoryreset") {
      localStorage.removeItem("akwaos.arg6MemoryComplete");

      state.arg6MemoryComplete = false;
      state.arg6MemoryRotations = [0, 0, 0, 0, 0, 0, 0, 0, 0];

      $("#arg6-memory-game")?.classList.add("is-hidden");
      $("#arg6-memory-result")?.classList.add("is-hidden");
      $("#arg6-memory-game .arg6-memory-game__panel")?.classList.remove("is-hidden");

      const btn = $("#arg6-memory-reconstruct");
      if (btn) btn.textContent = "RUN M-04 MEMORY RECONSTRUCTION";

      terminalPrint([
        "ARG6 MEMORY EPILOGUE RESET.",
        "M-04 MEMORY LOCKED.",
        "ISAAC SESSION STATE PRESERVED."
      ], "term-warn");
  ...
  const arg6Targets = ["A17", "ORV", "04", "IA"];

  const arg6Pools = [
    ["A03", "A11", "C17", "A17", "B04", "SEC"],
    ["SYS", "ORV", "NET", "DRV", "SDE", "AUX"],
    ["01", "07", "12", "04", "17", "26"],
    ["AZ", "IA", "AL", "IX", "OR", "AI"]
  ];
  ...
  function completeArg6Auth() {
    ...
    const status = $("#arg6-auth-status");
    if (status) status.textContent = "SESSION TOKEN RESTORED // ISAAC AZIM";
    ...
    appendSystemEvent("[--:--] ORV-04 service session restored / owner ISAAC AZIM");
  }
...
after A17 session termination.`
    ...
    if (key === "memory") {
      ...
      $("#arg6-memory-reconstruct")?.classList.remove("is-hidden");

      if (state.arg6MemoryComplete) {
        const btn = $("#arg6-memory-reconstruct");
        if (btn) btn.textContent = "OPEN RECOVERED M-04 MEMORY";
      }
    } else {
      view.classList.remove("arg6-file-view--important");
      $("#arg6-memory-reconstruct")?.classList.add("is-hidden");
    }
  ...
  const arg6MemoryTargets = [90, 0, 180, 90, 0, 270, 180, 0, 270];

  function calculateArg6MemorySync() {
    let correct = 0;

    state.arg6MemoryRotations.forEach((rotation, index) => {
      if (rotation === arg6MemoryTargets[index]) correct += 1;
    });

    return Math.round((correct / arg6MemoryTargets.length) * 100);
  }

  function updateArg6MemoryUI() {
    const sync = calculateArg6MemorySync();

    const label = $("#arg6-memory-sync");
    const fill = $("#arg6-memory-sync-fill");
    const feedback = $("#arg6-memory-feedback");

    if (label) label.textContent = `${sync}%`;
    if (fill) fill.style.width = `${sync}%`;

    state.arg6MemoryRotations.forEach((rotation, index) => {
      const tile = $(`.arg6-memory-tile[data-memory-tile="${index}"]`);
      if (!tile) return;

      const symbol = $("span", tile);
      if (symbol) symbol.style.transform = `rotate(${rotation}deg)`;

      tile.classList.toggle("is-synced", rotation === arg6MemoryTargets[index]);
    });

    if (feedback) {
      if (sync === 100) {
        feedback.textContent = "MEMORY PATH STABLE // RECONSTRUCTING...";
      } else if (sync >= 75) {
        feedback.textContent = "SIGNAL COHERENCE HIGH";
      } else if (sync >= 45) {
        feedback.textContent = "PARTIAL MEMORY PATH DETECTED";
      } else {
        feedback.textContent = "MEMORY PATH UNRESOLVED";
      }
    }

    if (sync === 100 && !state.arg6MemoryComplete) {
      completeArg6Memory();
    }
  }

  function randomizeArg6Memory() {
    const options = [0, 90, 180, 270];

    state.arg6MemoryRotations = arg6MemoryTargets.map((target, index) => {
      let rotation = options[Math.floor(Math.random() * options.length)];

      if (rotation === target) {
        rotation = options[(options.indexOf(rotation) + 1 + (index % 2)) % options.length];
      }
  ...
  function rotateArg6MemoryTile(index) {
    if (state.arg6MemoryComplete) return;

    state.arg6MemoryRotations[index] = (state.arg6MemoryRotations[index] + 90) % 360;
    updateArg6MemoryUI();

    makeTone(330 + index * 24, 0.045, 0.011, "square");
  }

  function openArg6Memory() {
    if (!state.arg6Unlocked) return;

    const game = $("#arg6-memory-game");
    if (!game) return;

    game.classList.remove("is-hidden");

    if (state.arg6MemoryComplete) {
      $("#arg6-memory-game .arg6-memory-game__panel")?.classList.add("is-hidden");
      $("#arg6-memory-result")?.classList.remove("is-hidden");
      return;
    }
  ...
  function closeArg6Memory() {
    $("#arg6-memory-game")?.classList.add("is-hidden");
  }

  function completeArg6Memory() {
    state.arg6MemoryComplete = true;
    localStorage.setItem("akwaos.arg6MemoryComplete", "1");

    makeTone(420, 0.11, 0.018, "sine");
    setTimeout(() => makeTone(610, 0.12, 0.016, "sine"), 110);
    setTimeout(() => makeTone(840, 0.16, 0.014, "sine"), 230);
    ...
    appendSystemEvent("[--:--] ORV-04 M-04 autonomous sensor memory recovered");
  }

  function resetArg6MemoryPuzzle() {
    if (state.arg6MemoryComplete) return;
    randomizeArg6Memory();
    clickTone();
  }
  ...
  function setupArg6() {
    $("#arg6-orv-entry")?.addEventListener("click", openArg6);
    ...
    $("#arg6-memory-reconstruct")?.addEventListener("click", openArg6Memory);
    $("#arg6-memory-close")?.addEventListener("click", closeArg6Memory);
    $("#arg6-memory-result-close")?.addEventListener("click", closeArg6Memory);
    $("#arg6-memory-reset-grid")?.addEventListener("click", resetArg6MemoryPuzzle);

    $$(".arg6-memory-tile").forEach(tile => {
      tile.addEventListener("click", () => {
        rotateArg6MemoryTile(Number(tile.dataset.memoryTile));
      });
    });
  }
