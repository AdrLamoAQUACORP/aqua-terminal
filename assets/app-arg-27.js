...
after A17 session termination.`
  ...
  function stopArg6DefenseTimers() {
    if (state.arg6DefenseTimer) {
      clearInterval(state.arg6DefenseTimer);
      state.arg6DefenseTimer = null;
    }

    if (state.arg6DefenseTick) {
      clearInterval(state.arg6DefenseTick);
      state.arg6DefenseTick = null;
    }

    if (state.arg6SweepTimer) {
      clearInterval(state.arg6SweepTimer);
      state.arg6SweepTimer = null;
    }
  }

  function arg6LiveMessage(text) {
    const log = $("#arg6-adrian-live-log");
    if (!log) return;
  ...
  function updateArg6DefenseUI() {
    const security = Math.max(0, Math.min(100, state.arg6SecurityLock));
    const link = Math.max(0, Math.min(100, state.arg6AdrianLinkStrength));

    const secValue = $("#arg6-security-lock-value");
    const secFill = $("#arg6-security-lock-fill");
    const linkValue = $("#arg6-adrian-link-value");
    const linkFill = $("#arg6-adrian-link-fill");
    const countdown = $("#arg6-adrian-countdown");

    if (secValue) secValue.textContent = `${Math.round(security)}%`;
    if (secFill) secFill.style.width = `${security}%`;
    if (linkValue) linkValue.textContent = `${Math.round(link)}%`;
    if (linkFill) linkFill.style.width = `${link}%`;

    if (countdown) {
      const minutes = Math.floor(state.arg6DefenseSeconds / 60);
      const seconds = state.arg6DefenseSeconds % 60;
      countdown.textContent = `${String(minutes).padStart(2, "0")}:${String(seconds).padStart(2, "0")}`;
    }

    const marker = $("#arg6-sweep-marker");
    if (marker) marker.style.left = `${state.arg6SweepPosition}%`;
  }
  ...
  function resetArg6DefenseRun() {
    ...
    state.arg6AdrianLinkStrength = 72;
    state.arg6DefenseSeconds = 60;
    state.arg6SweepPosition = 0;
    state.arg6ActionLocked = false;
    ...
    if (log) {
      log.innerHTML = `
        <p>&gt; Они увидели переход.</p>
        <p>&gt; Не закрывай окно.</p>
        <p>&gt; Мне нужна минута.</p>
      `;
    }

    const status = $("#arg6-route-status");
    if (status) status.textContent = "KEEP A.LAMO LINK ALIVE";
  ...
  function startArg6Defense() {
    ...
    state.arg6DefenseTimer = setInterval(() => {
      if (!state.arg6AdrianDefenseRunning) return;

      state.arg6DefenseSeconds -= 1;

      if (state.arg6DefenseSeconds === 43) {
        arg6LiveMessage("Перебрось канал через MAIL, если связь начнёт падать.");
      }

      if (state.arg6DefenseSeconds === 31) {
        arg6LiveMessage("Я почти закончила. Не дай им закрыть сессию.");
      }

      if (state.arg6DefenseSeconds === 18) {
        arg6LiveMessage("Ещё немного.");
      }

      if (state.arg6DefenseSeconds === 7) {
        arg6LiveMessage("Не отпускай канал.");
      }

      if (state.arg6DefenseSeconds <= 0) {
        completeArg6Defense();
        return;
      }
    ...
    state.arg6DefenseTick = setInterval(() => {
      if (!state.arg6AdrianDefenseRunning) return;

      state.arg6SecurityLock += state.arg6SecurityLock >= 72 ? 1.05 : 0.68;
      state.arg6AdrianLinkStrength -= state.arg6AdrianLinkStrength <= 35 ? 0.82 : 0.46;

      if (state.arg6SecurityLock >= 100 || state.arg6AdrianLinkStrength <= 0) {
        failArg6Defense();
        return;
      }
    ...
    state.arg6SweepTimer = setInterval(() => {
      if (!state.arg6AdrianDefenseRunning) return;

      state.arg6SweepPosition += 5;
      if (state.arg6SweepPosition > 100) state.arg6SweepPosition = 0;

      if (state.arg6SweepPosition >= 74 && state.arg6SweepPosition <= 84) {
        document.body.classList.add("arg6-sweep-hot");
      } else {
        document.body.classList.remove("arg6-sweep-hot");
      }
  ...
  function lockArg6ActionCooldown() {
    if (state.arg6ActionLocked) return false;

    state.arg6ActionLocked = true;
    const node = $("#arg6-action-cooldown");
    if (node) node.textContent = "ACTION COOLDOWN";

    setTimeout(() => {
      state.arg6ActionLocked = false;
      if (node) node.textContent = "ACTIONS READY";
    }, 1050);

    return true;
  }

  function maskArg6Session() {
    if (!state.arg6AdrianDefenseRunning || !lockArg6ActionCooldown()) return;

    state.arg6SecurityLock = Math.max(0, state.arg6SecurityLock - 12);
    state.arg6AdrianLinkStrength = Math.max(0, state.arg6AdrianLinkStrength - 3);

    const status = $("#arg6-route-status");
    if (status) status.textContent = "SESSION MASKED // LOCK -12 / LINK -3";

    clickTone();
    updateArg6DefenseUI();
  }

  function relayArg6Session() {
    if (!state.arg6AdrianDefenseRunning || !lockArg6ActionCooldown()) return;

    state.arg6AdrianLinkStrength = Math.min(100, state.arg6AdrianLinkStrength + 14);
    state.arg6SecurityLock = Math.min(100, state.arg6SecurityLock + 4);

    const status = $("#arg6-route-status");
    if (status) status.textContent = "LINK RELAYED // LINK +14 / LOCK +4";

    clickTone();
    updateArg6DefenseUI();
  }

  function handleArg6DefenseRoute(route) {
    if (!state.arg6AdrianDefenseRunning) return;

    const status = $("#arg6-route-status");

    if (route === "A17") {
      state.arg6SecurityLock = Math.max(0, state.arg6SecurityLock - 5);
      if (status) status.textContent = "LOCAL LOOP // LOCK -5";
      clickTone();
    } else if (route === "MAIL") {
      state.arg6AdrianLinkStrength = Math.min(100, state.arg6AdrianLinkStrength + 10);
      state.arg6SecurityLock = Math.min(100, state.arg6SecurityLock + 2);
      if (status) status.textContent = "MAIL RELAY // LINK +10 / LOCK +2";
      clickTone();
    } else if (route === "ARCH") {
      state.arg6SecurityLock = Math.max(0, state.arg6SecurityLock - 8);
      state.arg6AdrianLinkStrength = Math.max(0, state.arg6AdrianLinkStrength - 2);
      if (status) status.textContent = "ARCHIVE CACHE MASK // LOCK -8";
      clickTone();
    } else if (route === "ALAMO") {
      state.arg6AdrianLinkStrength = Math.min(100, state.arg6AdrianLinkStrength + 7);
      if (status) status.textContent = "A.LAMO LINK STABILIZED // +7";
      clickTone();
    } else if (route === "SEC") {
      state.arg6SecurityLock = Math.min(100, state.arg6SecurityLock + 9);
      if (status) status.textContent = "SEC-C HOT ROUTE // LOCK +9";
      errorTone();
    } else if (route === "WATCH") {
      state.arg6SecurityLock = Math.min(100, state.arg6SecurityLock + 17);
      if (status) status.textContent = "WATCHDOG CONTACT // LOCK +17";
      arg6LiveMessage("Не трогай WATCHDOG.");
      errorTone();
    }
  ...
  function failArg6Defense() {
    stopArg6DefenseTimers();

    state.arg6AdrianDefenseRunning = false;
    state.arg6SecurityLock = 100;
    updateArg6DefenseUI();

    document.body.classList.add("arg6-defense-failed");

    const status = $("#arg6-route-status");
    if (status) status.textContent = "SESSION TERMINATED // RETRYING";

    arg6LiveMessage("Нет. Ещё раз.");
    arg3ErrorBurst();
  ...
  function completeArg6Defense() {
    ...
    state.arg6SecurityLock = Math.min(state.arg6SecurityLock, 67);
    state.arg6AdrianLinkStrength = Math.max(state.arg6AdrianLinkStrength, 44);

    updateArg6DefenseUI();

    const status = $("#arg6-route-status");
    if (status) status.textContent = "A.LAMO LINK PRESERVED";
  ...
  function setupArg6() {
    ...
    $("#arg6-mask-session")?.addEventListener("click", maskArg6Session);
    $("#arg6-relay-session")?.addEventListener("click", relayArg6Session);
    $("#arg6-defense-exit")?.addEventListener("click", closeArg6DefenseSequence);

    $$(".arg6-route-node").forEach(node => {
      node.addEventListener("click", () => handleArg6DefenseRoute(node.dataset.arg6Route));
    });
  ...
  function setupMailNotification() {
    $("#mail-notification")?.addEventListener("click", () => {
      if (state.mailNotificationTimer) {
        clearTimeout(state.mailNotificationTimer);
