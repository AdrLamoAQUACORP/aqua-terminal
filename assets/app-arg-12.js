...
REF: BIO-CACHE / AQ-S17-441`,
  ...
  function createArg3ErrorWindows() {
    ...
    const errors = [
      ["SECURITY WATCHDOG", "UNAUTHORIZED WRITE"],
      ["AUDIT DAEMON", "CLEARANCE VIOLATION"],
      ["MEMORY CONTROL", "RESIDUAL CACHE OPEN"],
      ["BIO-CACHE", "PROFILE MISMATCH"],
      ["RESEARCH INDEX", "RESTRICTED QUERY"],
      ["SYSTEM", "TERMINATION REQUEST"]
    ];

    errors.forEach((item, i) => {
      const w = document.createElement("div");
      w.className = "arg3-error-window";
      w.style.left = `${8 + ((i * 17) % 64)}%`;
      w.style.top = `${10 + ((i * 13) % 60)}%`;
      w.style.animationDelay = `${i * 80}ms`;
      w.innerHTML = `
        <div class="arg3-error-window__bar">${item[0]} <span>×</span></div>
        <div class="arg3-error-window__body">
          <b>ERROR ${String(71 + i * 9).padStart(3, "0")}</b>
          <span>${item[1]}</span>
        </div>
      `;
      layer.appendChild(w);
  ...
  function setArg3DefensePrompt() {
    ...
    const cmd = arg3DefenseCommands[state.arg3CommandIndex];
    prompt.textContent = cmd || "PURGE INTERRUPTED";
  ...
  function submitArg3DefenseCommand(raw) {
    const expected = arg3DefenseCommands[state.arg3CommandIndex];
    if (!expected) return;
    ...
    if (normalized === expected) {
      ...
      setArg3PurgeProgress(state.arg3PurgeProgress - 19);

      if (status) {
        status.textContent = `COMMAND ACCEPTED // PURGE ROLLBACK ${state.arg3CommandIndex}/${arg3DefenseCommands.length}`;
      }
      ...
      if (state.arg3CommandIndex >= arg3DefenseCommands.length) {
        completeArg3Defense();
        return;
      }
    ...
    setArg3PurgeProgress(state.arg3PurgeProgress + 5);
    if (status) status.textContent = "COMMAND REJECTED // PURGE ACCELERATED";
    if (input) {
      input.value = "";
  ...
  function beginArg3Defense() {
    ...
    setArg3PurgeProgress(18);
    setArg3DefensePrompt();

    if (status) status.textContent = "ADRIAN TRACE: RECOVERY COMMANDS FOUND";
    input.disabled = false;
    input.value = "";
    setTimeout(() => input.focus(), 100);

    const alert2 = $("#arg3-alert-2");
    const alert3 = $("#arg3-alert-3");
    alert2?.classList.add("is-dim");
    alert3?.classList.add("is-dim");

    scheduleArg3Alarm(() => alert2?.classList.remove("is-dim"), 550);
    scheduleArg3Alarm(() => alert3?.classList.remove("is-dim"), 1050);

    state.arg3PurgeTimer = setInterval(() => {
      if (!state.arg3AlarmRunning) return;
      setArg3PurgeProgress(state.arg3PurgeProgress + 0.55);
  ...
  function triggerArg3SecurityEvent() {
    ...
    const alarmInterval = setInterval(() => {
      if (burst >= 7 || !state.arg3AlarmRunning) {
